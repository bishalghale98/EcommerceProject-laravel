                            ...
                            @if ($product->images)
                                @foreach (explode(',', $product->images) as $gimage)
                                    <div class="item gitems">
                                        <img src="{{ asset('uploads/products/' . trim($gimage)) }}"
                                            alt="{{ $product->name }}">
                                    </div>
                                @endforeach
                            @endif
    ...
    <script>
        $(function() {
            ...
            $("#gFile").on("change", function(e) {
                const gphotos = this.files; // Get all selected files
                const galUpload = $("#galUpload"); // Correct selector reference

                $.each(gphotos, function(key, val) {
                    galUpload.prepend(
                        `<div class="item gitems"><img src="${URL.createObjectURL(val)}" alt="Uploaded Image" /></div>`
                    );
                });
            });
